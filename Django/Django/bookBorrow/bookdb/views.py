...
from .models import Member, BookInfo, Publisher, Category, Book, Borrow, Review
...
def borrow_books(request):
    ...
    failed_borrows = []
    today = date.today()
    due_date = today + timedelta(days=14) #기본 대여 기간 14일
        ...
        with transaction.atomic():
            for isbn in isbn_list:
                ...
                available_copy = Book.objects.select_for_update().filter(
                    ...
                    status="대여가능"    #status가 '대여가능' 상태여야 함
...
def return_book(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            book_to_return.status = "대여가능" # 'status' 필드 사용
            book_to_return.save()
            ...
            if today > borrow.due_date:
                ...
                member.status = "대여정지"
                member.overdue_end_date = today + timedelta(days=7) #7일간 대여 정지
                member.save()
...
def admin_update_book_copy(request, book_manage_id):
    ...
    try:
        ...
        #상태 변경
        #대여가능, 대여중, 대여불가를 문자열로 해서 정확히 입력 안하고 띄어쓰기 등이 들어가면 제 3의 상태가 되는 문제 있음.
        #입력값 검사 필요, 숫자로 바꾸기? 
        if 'status' in data:
        ...
        return JsonResponse({"message": f"도서(ID:{book_manage_id}) 상태가 '{book_copy.status}'로 변경되었습니다."}, status=200)
...
def admin_delete_category(request, category_id):
    ...
    try:
        category = Category.objects.get(category_id=category_id)
        category.delete()
        #BookInfo의 category가 on_delete=models.SET_NULL로 설정되어 있으므로 카테고리를 삭제하면 해당 책들의 카테고리는 NULL이 됨.
...
def admin_delete_publisher(request, publisher_id):
    ...
    try:
        ...
        #삭제 실행
        #BookInfo의 publisher 필드는 on_delete=models.SET_NULL이라서 출판사를 삭제하면 이 출판사의 책들은 NULL이 됨
        publisher.delete()
