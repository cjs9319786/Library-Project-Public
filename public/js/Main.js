...
async function display_Detail(isbn) {
  ...
  document.getElementById("my_review_actions").innerHTML = "";  // 버튼 초기화
  await loadMyReviewStatus(isbn);                               // 내 상태 확인 (버튼 생성)
  await loadAllReviews(isbn);                                   // 전체 리뷰 목록 로드
}
...
async function submitReview() {
    ...
    if (mode === 'create') {
        ...
        bodyData = { rating: rating, content: content }; // update는 isbn 불필요
    }
...
async function deleteReview() {
    ...
    const token = localStorage.getItem("authToken");
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/reviews/delete/${myReviewId}/`, {
            method: "POST", // views.py가 DELETE 대신 POST를 쓴다면 POST로 유지
            headers: { "Authorization": `Bearer ${token}` }
        });
